...
class Tensorflow(Package):
    ...
    def patch(self):
        if ('cxxstd=17' in self.spec ):
            install(os.path.join(os.path.dirname(__file__), 'patch/build_all_linux_cxx17.sh'), 'tensorflow/contrib/makefile/build_all_linux.sh')
        else:
            install(os.path.join(os.path.dirname(__file__), 'patch/build_all_linux.sh'), 'tensorflow/contrib/makefile/build_all_linux.sh')

        install(os.path.join(os.path.dirname(__file__), 'patch/install_all.sh'), 'install_all.sh')
        install(os.path.join(os.path.dirname(__file__), 'patch/rename_eigen.sh'), 'tensorflow/contrib/makefile/rename_eigen.sh')
