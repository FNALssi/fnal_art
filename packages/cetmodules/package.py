...
class Cetmodules(CMakePackage):
    ...
    variant("versioned-docs", default=False, description="build versioned docs with a landing page")
    ...
    def cmake_args(self):
        ...
        if not any([ self.spec.variants[doc_opt].value
            ...
            options += [ self.define("{0}_SPHINX_DOC_PUBLISH_VERSION_BRANCH".format(self.name),
                                     self.spec.version),
                         self.define("{0}_SPHINX_DOC_PUBLISH_ROOT".format(self.name),
